    ...
    public class SettingsFrame : Frame
    {
        ...
        public override IEnumerable<byte> Payload
        {
            get
            {
                ...
                {
                    data.AddRange(BitConverter.GetBytes((ushort)0x1).EnsureBigEndian());
                    data.AddRange(BitConverter.GetBytes(HeaderTableSize.Value).EnsureBigEndian());
                }
        ...
        public override void ParsePayload(byte[] payloadData, FrameHeader frameHeader)
        {
            ...
            for (var i = 0; i < payloadData.Length; i += 6)
            {
                ...
                {
                    value = BitConverter.ToUInt32(payloadData.Skip(i+2).Take(4).Reverse().ToArray(), 0);
                    parameter = BitConverter.ToUInt16(payloadData.Skip(i).Take(2).Reverse().ToArray(), 0);
                }
                else
                {
                    value = BitConverter.ToUInt32(payloadData, i + 2);
                    parameter = BitConverter.ToUInt16(payloadData, i);
                }
                switch (parameter)
                {
                    case 0x1:
