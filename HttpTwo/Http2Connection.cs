...
namespace HttpTwo
{
    ...
    public class Http2Connection
    {
        ...
        public async Task Connect()
        {
            ...
            await QueueFrame(w).ConfigureAwait(false);

            // Send out preface data
            var prefaceData = System.Text.Encoding.ASCII.GetBytes(ConnectionPreface);
            var settingsFrameArray = s.ToBytes().ToArray();
            var windowUpdateFrameArray = w.ToBytes().ToArray();

            //var initFrames = new List<byte>();
            //initFrames.AddRange(prefaceData);
            //initFrames.AddRange(settingsFrameArray);
            //initFrames.AddRange(windowUpdateFrameArray);

            //var packetFrames = initFrames.ToArray();

            ////packetFrames[25] = 0x11;
            ////packetFrames[24] = 0x10;
            ////packetFrames[30] = 0x20;
            //for (int i = 0; i < prefaceData.Length; i++)
            //    clientStream.WriteByte(prefaceData[i]);
        ...
        async void Read()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    string v = Encoding.Default.GetString(b);
                }
                catch
                {
                    rx = -1;
